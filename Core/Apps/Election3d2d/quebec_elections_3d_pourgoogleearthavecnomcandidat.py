...
- Fait la jointure sur les NOMS de circonscription (pas les codes,
  qui sont incompatibles entre shapefile et CSV)
- Normalise les noms pour matcher (accents, tirets, etc.)

Prérequis :
    pip install geopandas simplekml pandas requests unidecode
...
def preparer_resultats(csv_candidats):
    ...
    circonscription : parti et candidat gagnant, parti et candidat 2e,
    pourcentages et marge.
    """
    ...
    col_nom_candidat = "Nom"
    col_prenom_candidat = "Prénom"
    ...
    # Les colonnes nom/prenom sont optionnelles (pour retro-compatibilite)
    has_noms = col_nom_candidat in df.columns and col_prenom_candidat in df.columns

    print(f"  [OK] Colonnes utilisees : circo={col_circo_num}, parti={col_parti}, "
          f"votes={col_votes}, noms candidats={has_noms}")

    df[col_votes] = pd.to_numeric(df[col_votes], errors="coerce").fillna(0)

    def format_candidat(row):
        """Formate 'Prenom Nom' proprement."""
        if not has_noms:
            return ""
        prenom = str(row.get(col_prenom_candidat, "") or "").strip()
        nom = str(row.get(col_nom_candidat, "") or "").strip()
    ...
    for num_circo, groupe in df.groupby(col_circo_num):
        ...
        # Trier les candidats de cette circo par votes decroissants
        groupe_trie = groupe.sort_values(col_votes, ascending=False).reset_index(drop=True)
        if len(groupe_trie) == 0:
            continue

        # 1er : le gagnant
        gagnant = groupe_trie.iloc[0]
        votes_gagnant = float(gagnant[col_votes])
        pct_gagnant = (votes_gagnant / total) * 100
        parti_gagnant_brut = str(gagnant[col_parti])
        candidat_gagnant = format_candidat(gagnant)

        # 2e : le suivant
        if len(groupe_trie) > 1:
            deuxieme = groupe_trie.iloc[1]
            votes_deuxieme = float(deuxieme[col_votes])
            pct_deuxieme = (votes_deuxieme / total) * 100
            parti_deuxieme_brut = str(deuxieme[col_parti])
            candidat_deuxieme = format_candidat(deuxieme)
        else:
            votes_deuxieme = 0
            pct_deuxieme = 0
            parti_deuxieme_brut = ""
        ...
        resultats.append({
            ...
            "parti_deuxieme": simplifier_parti(parti_deuxieme_brut) if parti_deuxieme_brut else "",
            "parti_deuxieme_brut": parti_deuxieme_brut,
            "candidat_deuxieme": candidat_deuxieme,
            "pourcentage_deuxieme": pct_deuxieme,
            "votes_deuxieme": int(votes_deuxieme),
...
def creer_kml(gdf):
    ...
    for _, row in gdf.iterrows():
        ...
        # Nouvelles infos : candidat gagnant, parti 2e, candidat 2e
        candidat_g = row.get("candidat_gagnant")
        candidat_g = str(candidat_g) if pd.notna(candidat_g) and candidat_g else "-"

        parti_2_raw = row.get("parti_deuxieme")
        parti_2 = str(parti_2_raw) if pd.notna(parti_2_raw) and parti_2_raw else "-"

        candidat_2 = row.get("candidat_deuxieme")
        candidat_2 = str(candidat_2) if pd.notna(candidat_2) and candidat_2 else "-"

        pct_2 = row.get("pourcentage_deuxieme")
        pct_2 = float(pct_2) if pd.notna(pct_2) else 0.0

        votes_2 = row.get("votes_deuxieme")
        votes_2 = int(votes_2) if pd.notna(votes_2) else 0
        ...
        desc = (
            f"<![CDATA["
            f"<h3>{nom}</h3>"
            f"<p style='color:#666;font-size:11px'>Code CEP : {code}</p>"
            f"<hr style='border:none;border-top:1px solid #ddd'>"
            f"<p style='margin-top:8px'><b>🥇 1er - {parti}</b><br>"
            f"{candidat_g}<br>"
            f"{pct:.1f}% ({votes_g:,} votes)</p>"
            f"<p><b>🥈 2e - {parti_2}</b><br>"
            f"{candidat_2}<br>"
            f"{pct_2:.1f}% ({votes_2:,} votes)</p>"
            f"<hr style='border:none;border-top:1px solid #ddd'>"
...
def main():
    print("=" * 60)
    print(" ELECTIONS 3D - v2 (avec noms de circo)")
    print("=" * 60)
